...
interface ConfigState {
  ...
  load: () => Promise<void>
  /** llmApiKey 传 undefined 表示不改动已存的那把 */
  save: (config: AppConfig, llmApiKey?: string) => Promise<void>
...
export const useConfig = create<ConfigState>((set) => ({
  ...
  load: async () => {
    ...
    // 界面只会永远停在「载入配置中」
    try {
      const { config, hasLlmApiKey, configExists } = await window.api.loadConfig()
      set({ config, hasLlmApiKey, configExists, loaded: true, loadError: null })
    } catch (err) {
      set({ loadError: `配置载入失败：${String(err)}` })
    }
  },

  save: async (config, llmApiKey) => {
    try {
      await window.api.saveConfig({ config, llmApiKey })
      set((s) => ({
        config,
