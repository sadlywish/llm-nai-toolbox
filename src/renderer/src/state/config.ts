...
interface ConfigState {
  ...
  hasNaiToken: boolean
  /** 是否存过 Danbooru API Key，同 hasLlmApiKey */
  ...
  load: () => Promise<void>
  /** 三把钥匙传 undefined 都表示不改动已存的那把 */
  save: (config: AppConfig, llmApiKey?: string, naiToken?: string, danbooruApiKey?: string) => Promise<void>
...
export const useConfig = create<ConfigState>((set) => ({
  ...
  load: async () => {
    ...
    // 界面只会永远停在「载入配置中」
    try {
      const { config, hasLlmApiKey, hasNaiToken, hasDanbooruApiKey, configExists } = await window.api.loadConfig()
      set({ config, hasLlmApiKey, hasNaiToken, hasDanbooruApiKey, configExists, loaded: true, loadError: null })
    } catch (err) {
      set({ loadError: `配置载入失败：${String(err)}` })
    }
  },

  save: async (config, llmApiKey, naiToken, danbooruApiKey) => {
    try {
      ...
      set((s) => ({
        ...
        hasDanbooruApiKey: danbooruApiKey === undefined ? s.hasDanbooruApiKey : danbooruApiKey !== '',
        saveError: null,
      }))
    } catch (err) {
