...
**每次发送都是全新一轮**，不累积历史。想在现有内容上改，打开「修改模式」开关——它会把编辑器当前内容作为编辑输入一起送出去。

底部交互区只滚动显示**工具调用记录**（模型现在在查什么）。完整对话、thinking、token 用量都完整落盘，去历史弹窗看。
...
|---|---|
| 不覆盖 | 原样写入 |
| 用选用的画风覆盖 | 丢弃模型返回的，写入预设下拉选中的那条 |
| 用当前编辑框里的画风覆盖 | 丢弃模型返回的，保留 `artist` 段现有内容 |
