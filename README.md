...
指令区固定在窗口最下面，上面的提示词、参数、角色整体滚动。输入指令，下面一排是和 LLM 交互的开关——多角色（关闭 / 位置由模型安排 / 手动指定坐标）、在现有内容上修改、透明背景、画风。Ctrl+Enter 发送。

**每次发送都是全新一轮**，不累积历史。想在现有内容上改，打开「在现有内容上修改」——编辑器、负面词、参数、角色的当前内容会一起送出去。

发送后，日志抽屉从指令区顶边向上展开，照 koishi 日志一行一条滚动（本轮工具集、每条搜索的匹配、token 用量……）；点遮罩收起，这一轮照常跑，指令区顶上的细栏里有状态、「查看日志」与「中止」。成功时抽屉自动收起，把最终参数直接回填进编辑器、负面词、参数区与角色区；模型没给参数、请求失败、中止时编辑器内容一律不动。seed 不归模型管，由参数区决定。
...
|---|---|
| 不覆盖 | 原样写入 |
| 用预设画风覆盖 | 丢弃模型返回的，写入当前预设画风 |
| 用当前 artist 块覆盖 | 丢弃模型返回的，保留 `artist` 段现有内容 |

画风不会发给模型：后两档在回填前直接覆盖 `artist`。

预设在顶栏的「画风维护」里维护：左边列表（可拖动排序），右边编辑名称和标签，改动即保存。在那里「选为预设画风」，指令区只显示当前预设名和「选择预设…」；「覆盖画风」把一条画风直接写进工作台的 artist 块；「从工作台提取」把 artist 块存成新画风。

### 出图

顶部工具栏填跑图次数、点「生成」；指令区打开「回填后自动生成」时，LLM 回填成功后直接开跑。出图弹窗里一张张出现，单击看溯源信息（本工具参数 / 图片元信息），双击看原图。「复制信息」把那张图的整套参数写回参数区，seed 取图片里的并改成固定。

左侧历史竖栏列出最近几天的每一轮，点开回看。遇到 429（别的客户端在用同一账号）暂停等你处理；Token 失效或点数不足直接中止整批。

### 右侧 WIKI 区

工具栏最右端的「WIKI」按钮展开或整体收起右侧竖栏；收起时跟随光标等功能全部暂停。

- 顶部「画师 / 标签」切换数据源，「跟随光标」开着时，光标停在正向提示词（整图、角色）的词上 0.4 秒后自动查询，并按词是否是画师自动切换数据源。
- 搜索框：标签用本地标签库联想（带中文别名）；画师额外用 Danbooru 按别名、主页链接检索。
- 词条头显示分类、帖子数、本地中文别名；画师另有封禁/删除标注、别名与主页。「+ 加入」把这个词插回上次光标所在的位置（画师自动加 `artist:`，下划线换空格）。
- wiki 正文按 DText 渲染（内链点了在栏内跳转，外链用系统浏览器打开），See also 单独成一行可点的标签。
- 例图：标签取评分最高 6 张，画师按新/中/旧三档各 6 张；点开用查看器看原图。
- 设置里可填 Danbooru 用户名与 API Key（可不填，匿名也能用）。
